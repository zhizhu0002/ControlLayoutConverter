...
- 移除转换结果的一次 `JSONObject(...).toString()` 重序列化：引擎输出原样透传。顺带修掉一个正确性问题——ZL2 颜色是 signed 64-bit 打包整数（如 `-9223372036854775808`），经 JSON 解析再序列化会被改写
- JSON 输入框改用惰性文本 API（`TextFieldState` + `lineLimits`）。此前 Miuix `TextField(value: String)` 会在每次输入变化时重排整段文本，粘贴大布局明显卡顿
- 预览改为虚拟化列表：内容全量保留，但只渲染可见行

### 体积 📦

- `libcc.so` 在 APK 内改用 deflate 压缩存放（`useLegacyPackaging = true`）：release 包 5.68MB → 约 3.6MB。原生库二进制逐字节未改动（CRC-32 校验一致），安装后由系统解压到应用数据目录

### 横屏适配 📐

- 导航栏由底部改为**左侧 `NavigationRail`**：图标在上、文字在下，垂直居中，去掉展开/收起按钮
- 主页横屏改为左右双栏（左＝格式与输入，右＝输出结果），两栏各自独立滚动
- 关于页与第三方许可证页横屏限宽居中，避免宽屏下行长过长

### 视觉效果 ✨

- 导航栏毛玻璃改用官方 `miuix-blur`。因 `minSdk 26 < 33`（RuntimeShader 需 API 33），按官方文档以 `uses-sdk overrideLibrary` 放行合并，并用运行时能力检查门控全部 blur 代码路径，低版本自动回退为半透明底色
- 新增深浅色模式：**自动 / 浅色 / 深色**三档，位于「关于 → 主题」板块，标题「颜色」（Miuix `WindowDropdownMenu`），选择结果持久化
...
- **显示 Miuix `WindowDialog` 时崩溃**（`IllegalStateException: No NavigationEventDispatcher was provided via LocalNavigationEventDispatcherOwner`）：Miuix 的 `DialogContentLayout` 会调用 `NavigationBackHandler`，而旧版 `androidx.activity` 不提供该 owner。`androidx.activity` 1.10.0 → 1.12.0 解决
